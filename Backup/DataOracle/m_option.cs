    ...
    public class m_option
    {
        ...
        public DataTable f_get_KiemtraTong(DateTime tungay, DateTime denngay)
        {
            ...
            try
            {
                string userid = AccessData.m_userid;
                string sql;
                sql = "   select  to_char(ngay,'dd/mm/yyyy'),id,thuoc, sotien-(vtth+mau+xetnghiem+cdha+dvktthuong+dvktcao+cpvc+tdcn+giuong+khac+khambenh) ";
                sql += " from bv115.v_bhytll ";
                sql += " where  sotien<>mau+xetnghiem+cdha+dvktthuong+dvktcao+cpvc+tdcn+giuong+khac+thuoc+khambenh+vtth + thuock";
                sql += " and  to_char(ngay,'yyyyMMdd') between '{tungay}' and '{denngay}'  ";
