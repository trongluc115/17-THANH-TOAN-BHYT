...
{
    public class CThanhToanBHYTOracleEdit
    {
        ...
        #region phuong thuc
        public CThanhToanBHYTOracleEdit()
        {
        ...
        public DataTable kiemtratrung_trongngay(string loaibn, DateTime Ngay)
        {
            ...
            bool kq = false;
            sql = " select to_char(a.ngayvao,'dd/mm/yyyy'),a.mabn,count(*) from " + schema + ".e_BHYTds a ";
            sql += " join " + schema + ".e_BHYTll b on a.id=b.id ";
        ...
        public DataSet f_loadDanhSachDaDuyet(string tungay, string denngay,string dieukien)
       {
        ...
        //    sql=sql.Replace("'{denngay}'",f_formatNgay(denngay));
           ...
           string sql = "    select  b.SOPHIEU SOPHIEU,TO_CHAR( A.MABN) MABN,bn.HOTEN HOTEN, a.MAICD ICD10, ";
           ...
           sql += "  A.NGAYVAO NGAYVAO, A.NGAYRA NGAYRA,ROUND(TO_DATE(TO_CHAR(A.NGAYRA,'DD/MM/YYYY'),'DD/MM/YYYY')-TO_DATE(TO_CHAR(A.NGAYVAO,'DD/MM/YYYY'),'DD/MM/YYYY'),1)+1 SONGAY,A.CHANDOAN CHANDOAN,b.THUOC THUOC,b.MAU MAU,b.XETNGHIEM XETNGHIEM, ";
           ...
           sql += " b.TDCN THAMDOCHUCNANG,a.SOTHE SOTHEBHYT,to_char(a.tungay,'DD/MM/YYYY') TUNGAY,to_char(a.denngay,'DD/MM/YYYY') DENNGAY, A.TRAITUYEN TRAITUYEN, A.ID ,A.MAKP MAKP ,A.DUYETKHTH,A.USERID";
           sql += "            from " + schema + ".e_BHYTDS a ";
           sql += " join " + schema + ".e_BHYTLL b on  a.id=b.id ";
        ...
        public DataSet f_getv_ttrvkp_ct_BV115(string MaBN,string idduyet)
        {
            ...
            string sqlcp = "           select ds.mavaovien,ds.mabn,to_char(cd.makp) MAKP,cd.makp idkhoa ,kp.tenkp , cd.madoituong, cd.ngay, case when vp.kythuat=0 then -1 else nhom.stt end  sttnhom,  ";
            sqlcp += "         case when vp.kythuat=0 then 'Dịch vu kỹ thuật cao chi phí lớn' else to_char(nhom.ten) end nhom,cd.mavp, to_char(vp.ten) ten,to_char(vp.dvt) DVT,cd.soluong,cd.dongia,vp.bhyt,vp.kythuat,2 tinhtong,cd.sotien sotien,cd.bhyttra bhyttra,cd.idnhombhyt idnhombhyt ,ll.tilethe,ll.tilehuong,ll.sophieu,ds.computer,ds.userid";
            sqlcp += "           from " + schema + ".e_bhytct cd  ";
            sqlcp += "           join " + schema + ".e_bhytds ds on ds.id=cd.id ";
            sqlcp += "           join " + schema + ".e_bhytll ll on ll.id=ds.id ";
            sqlcp += "           join medibv115.v_giavp vp on cd.mavp=vp.id ";
            sqlcp += "           join medibv115.v_loaivp loai on vp.id_loai=loai.id ";
            sqlcp += "           join medibv115.v_nhomvp nhom on loai.id_nhom=nhom.ma ";
            sqlcp += "           join medibv115.btdkp_bv kp on cd.makp=kp.makp ";
            ...
            sqlcp += "           union all ";
            sqlcp += "           select ds.mavaovien,ds.mabn,to_char(kp.makp) MAKP,cd.makp idkhoa,kp.tenkp tenkp ,cd.madoituong, cd.ngay, case when vp.kythuat=0 then -1 else nhomvp.stt end  sttnhom,  ";
            sqlcp += "           case when vp.kythuat=0 then 'Dịch vu kỹ thuật cao chi phí lớn' else to_char(nhomvp.ten) end nhom, cd.mavp mavp, to_char(vp.ten||' '||vp.hamluong) ten,to_char(vp.dang) dvt,cd.soluong,round(cd.dongia,2) dongia,vp.bhyt,vp.kythuat,1 tinhtong ,cd.sotien sotien,cd.bhyttra bhyttra,cd.idnhombhyt idnhombhyt,ll.tilethe,ll.tilehuong,ll.sophieu ,ds.computer,ds.userid";
            sqlcp += "           from " + schema + ".e_bhytct cd  ";
            sqlcp += "           join " + schema + ".e_bhytds ds on ds.id=cd.id ";
            sqlcp += "           join " + schema + ".e_bhytll ll on ll.id=ds.id ";
            ...
            string sqlGroup = " select MAVAOVIEN,MABN,MAKP,TENKP,MADOITUONG,STTNHOM,NHOM,MAVP,TEN,DVT,DONGIA DONGIA,BHYT,KYTHUAT, TINHTONG,sum(SOLUONG) SOLUONG,sum(kq.sotien) TONGTIEN, sum(BHYTTRA) BHYTTRA,sum(SOTIEN-BHYTTRA) BNTRA ,tilethe,tilehuong,sophieu,computer";
            sqlGroup += "           from ";
            sqlGroup += "           ( ";
            sqlGroup += sqlcp;
            sqlGroup += "           ) kq ";
            
            sqlGroup += "           group by MAVAOVIEN,MABN,MAKP,TENKP,MADOITUONG,STTNHOM,NHOM,MAVP,TEN,DVT,DONGIA,BHYT,KYTHUAT,TINHTONG,TILETHE,TILEHUONG,sophieu,computer ";
            ...
            sql += " 0 SBLTAMUNG,cd.sophieu||':'||cd.computer SOLUUTRU,  '' NGUOINHA,1 PHUONGPHAP,1 HINHTHUC,'' KETQUA,cd.computer KHU,'' NGAYPT,  0 TENPT,0	MASO,CD.SOPHIEU	SOPHIEU, ";
            ...
            sql += " left join DMNOICAPBHYT  ndk on ndk.mabv=ba.mabv     ";
            sql += " left join bv_tilebhyt tl on tl.kytu=substr(ba.sothe,3,1) and ba.traituyen=tl.traituyen    ";
        ...
        public DataSet f_loadBCMau79_80_DK(string tungay, string denngay, string loaibn, bool canngheo, string KTCAO, string ThuocUT,string dieukien)
        {
            ...
            sql += " join " + schema + ".e_bhytLL b on  a.id=b.id  ";
            sql += " join btdbn bn on a.mabn=bn.mabn  ";
            sql += " left join dmnoicapbhyt bv on bv.mabv=a.mabv ";//substr(a.sothe,-5,5) ";
            sql += " where to_char(b.ngay,'yyyymmdd') between '{tungay}' and '{denngay}' @dieukien";
            //if (canngheo)
            //{
            //    sql += " and substr(a.SOTHE,0,2) in ('CN') and substr(a.mabv,0,2) in ('79') and b.sotien-b.bhyt>0 ";
        ...
        public DataSet f_loadBCMau20(string tungay, string denngay, string dieukien, bool canngheo, string dsnhombhyt, string ThuocUT)
        {
            ...
            sql += " join " + schema + ".e_bhytll b on b.id=x.id ";
            sql += " join " + schema + ".e_bhytds a on a.id=b.id ";
        ...
        {
            string sql = "";
            sql = "	select nbhyt.ten tennhom,BHKP.KHOXUAT KHOXUAT,kq.mavp ,dm.ten tenvp,dm.hamluong,dm.tenhc,dm.dang donvi,dm.duongdung dang,dm.sodk QD05,nc.ten tennuoc,h.ten tenhang, kq.dongia,  ";	  
            ...
            sql += "	 from " + schema + ".e_bhytct ct  	  ";
            sql += "	 join " + schema + ".e_bhytll b on b.id=ct.id 	  ";
            sql += "	 join " + schema + ".e_bhytds a on a.id=b.id  	  ";
            sql+="	 where     to_char(b.ngay,'yyyymmdd') between '{tungay}' and '{denngay}' 	  ";
            ...
            sql += "	 group by nbhyt.ten ,BHKP.KHOXUAT,kq.mavp ,dm.ten , dm.hamluong,dm.tenhc,dm.dang,dm.duongdung,dm.sodk,nc.ten,h.ten,kq.dongia	  ";
        ...
        public DataSet f_loadBCMau21(string tungay, string denngay, string dieukien, bool VTYT, string dsnhombhyt, string ThuocUT)
        {
            ...
            sql += " where     to_char(b.ngay,'yyyymmdd') between '{tungay}' and '{denngay}' and nbhyt.id in (@nhombhyt)  and ct.madoituong=1 @dieukien";
        ...
        {
            string sql = "";
            sql = "	select nbhyt.ten tennhom,kq.mavp ,vp.ten tenvp, kq.dongia,	  ";
            sql+="	SUM(kq.slngoaitru) SLNGOAITRU,sum(kq.slnoitru) SLNOITRU,	  ";
            sql+="	 kq.dongia*SUM(kq.slngoaitru) THANHTIEN_NGOAITRU,	  ";
            sql+="	 kq.dongia*SUM(kq.slnoitru) THANHTIEN_NOITRU	  ";
            sql+="	from (	  ";
            ...
            sql += "	 inner join " + schema + ".v_nhombhyt nbhyt on nbhyt.id=kq.idnhombhyt 	  ";
            sql+="	 group by nbhyt.ten ,kq.mavp ,vp.ten , kq.dongia	  ";
        ...
        {
            string sql = " select b.sophieu,a.mabn mabn,LOWER(bn.HOTEN) HOTEN,";
            ...
            sql += " ,case when length(a.SOTHE)<20 THEN 'THU PHÍ' ELSE SUBSTR(a.SOTHE,1,15) END MATHE,A.MABV MA_DKBD,CASE(INSTR( a.MAICD,';')) ";
            sql += " WHEN -1 THEN a.MAICD  WHEN 0 THEN a.MAICD ELSE SUBSTR(A.MAICD,0, INSTR( a.MAICD,';')-1) END MABENH ";
            sql += " ,to_char(A.NGAYVAO,'dd/mm/yyyy') NGAYVAO,to_char(A.NGAYRA,'dd/mm/yyyy') NGAYRA ";
            sql += " ,ROUND(A.NGAYRA-A.NGAYVAO)+1 NGAYDTR ";
        ...
        public DataSet f_Export_Excel_M79_80(string tungay, string denngay, string dieukien)
        {
            ...
            sql += " WHEN -1 THEN a.MAICD  WHEN 0 THEN a.MAICD ELSE SUBSTR(A.MAICD,0, INSTR( a.MAICD,';')-1) END MABENH ";
            sql += " ,to_char(A.NGAYVAO,'dd/mm/yyyy') NGAYVAO,to_char(A.NGAYRA,'dd/mm/yyyy') NGAYRA ";
            sql += " ,ROUND(A.NGAYRA-A.NGAYVAO) NGAYDTRI ";
        ...
        public DataSet f_Export_ExcelXHH(string tungay, string denngay, string dieukien)
        {
            ...
            sql+=" SUM(case when kq.nhom='PTNSOI' then  KQ.SOLUONG*KQ.DONGIA ELSE 0 end)  as PTNSOI, ";
            sql+=" SUM(case when kq.nhom='KTC' then  KQ.SOLUONG*KQ.DONGIA ELSE 0 end)  as KTC, ";
            sql+=" SUM(case when kq.nhom='DSAKTC' then  KQ.SOLUONG*KQ.DONGIA ELSE 0 end)  as DSAKTC, ";
            sql+=" SUM(case when kq.nhom='DSATH' then  KQ.SOLUONG*KQ.DONGIA ELSE 0 end)  as DSATH, ";
            ...
            sql+=" from ( ";
            sql += " select b.sophieu,A.mabn,a.makp,e.viettat nhom,c.dongia,c.soluong ";
            sql += " from         " + schema + ".e_bhytds a ";
            sql += " inner join " + schema + ".e_bhytLL b on a.id=b.id ";
            sql += " inner join " + schema + ".e_bhytCT C on a.id=c.id ";
            sql += " inner join " + schema + ".V_GIAVPBH d on d.id=c.mavp ";
            sql += " inner join " + schema + ".nhomxhh e on d.id_type=e.id ";
            sql += " where e.xhh='XHH' and to_char(b.ngay,'yyyymmdd') between '{tungay}' and '{denngay}' @dieukien ";
            sql+=" ) kq ";
