...
//					}
        ...
        public string f_GetTime(string s_value)
        {   string s_time="";
            int pos = s_value.IndexOf(":");
            if (pos < 0)
            {
                switch (s_value.Length)
                {
                    case 1:
                        s_time = "0" + s_value + ":00";
                        break;
                    case 2:
                        ...
                        break;
                    case 3:
                        s_time = "0" + s_value.Substring(0, 1) + ":" + s_value.Substring(1, 2);
                        break;
                    case 4:
                        s_time = s_value.Substring(0, 2) + ":" + s_value.Substring(2, 2);
                        break;
                    
                }
            }
            else { 
                string hh=string.Format("{0:00}",int.Parse(s_value.Substring(0,pos)));
                string mm = string.Format("{0:00}", int.Parse(s_value.Substring(pos+1,s_value.Length-pos-1)));
                s_time = hh + ":" + mm;
            }
            return s_time;
        }
