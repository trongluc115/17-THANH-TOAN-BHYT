...
namespace DataOracle
{
    public class CThanhToanBHYTOracleNoiTru
    {
        ...
        public string f_loadICDFull( string mavaovien,DateTime Ngay)
        {
            ...
            sql += " union ";
            sql += " select ba.maicd chandoan,0 stt from {database}.benhanpk ba where ba.mavaovien='{mavaovien}' and  ba.maicd is not null";
            sql += " union ";
            ...
            sql += " ) kq group by chandoan,stt order by stt asc";
        ...
        public string f_loadICD_9_Full(string s_id)
        {
            string kq = "";
            string MaICD = "";
            sql = " select vp.maicd9 chandoan ";
            sql+=" from bv115.v_bhytds a ";
          //  sql+=" inner join bv115.v_bhytll b on a.id =b.id ";
            sql += " inner join bv115.v_bhytct c on a.id =c.id ";
            sql+=" inner join medibv115.v_giavp vp on c.mavp=vp.id ";
            sql+=" where a.id={s_id} ";
        ...
        public CThanhToanBHYT f_loadTT_BHYT_CLS(string MaVaoVien, DateTime Ngay,string s_mabn)
        {
            ...
            sql += " select 0 idtt,bh.maql,bh.sothe,bh.mabv,nc.tenbv,bh.denngay HSD,bh.tungay,bh.traituyen,0 sobienlai,0 loaibn ";
            sql += " from xxxxx.bhyt bh ";
            sql += " join dmnoicapbhyt nc on nc.mabv=bh.mabv ";
            sql += " where     bh.mabn='{s_mabn}' ";

            
            sql = sql.Replace("xxxxx", getdatabase(Ngay));
            sql = sql.Replace("{mavaovien}", MaVaoVien);
            sql = sql.Replace("{s_mabn}", s_mabn);
            DataSet dset = new DataSet();
            try
            {
                dset = data.get_data(sql);
        ...
        public DataSet f_loadDanhSachDaDuyet(string tungay, string denngay,string dieukien)
       {
        ...
        //    sql=sql.Replace("'{denngay}'",f_formatNgay(denngay));
           ...
           sql+=" b.VTTH VATTUYTE,b.VTYTTL VTYTTL,b.KHAMBENH TIENKHAM,b.GIUONG GIUONG,b.CPVC CHIPHIVC,b.KHAC KHAC, ";
        ...
        public DataSet f_getv_ttrvkp_ct_BV115(string MaBN,string idduyet)
        {
            ...
            sqlcp += "         case when vp.kythuat=0 then 'Dịch vu kỹ thuật cao chi phí lớn' else to_char(nhom.ten) end nhom,cd.mavp, to_char(vp.ten) ten,to_char(vp.dvt) DVT,cd.soluong,cd.dongia,vp.bhyt,vp.kythuat,2 tinhtong,cd.sotien sotien,cd.bhyttra bhyttra,cd.idnhombhyt idnhombhyt ,ll.tilethe,ll.tilehuong,ll.ubndtra MIEN";
            ...
            sqlcp += "           case when vp.kythuat=0 then 'Dịch vu kỹ thuật cao chi phí lớn' else to_char(nhomvp.ten) end nhom, cd.mavp mavp, to_char(vp.ten||' '||vp.hamluong) ten,to_char(vp.dang) dvt,cd.soluong,round(cd.dongia,2) dongia,vp.bhyt,vp.kythuat,1 tinhtong ,cd.sotien sotien,cd.bhyttra bhyttra,cd.idnhombhyt idnhombhyt,ll.tilethe,ll.tilehuong,LL.UBNDTRA MIEN ";
            sqlcp += "           from " + schema + ".v_bhytct cd  ";
            sqlcp += "           join " + schema + ".v_bhytds ds on ds.id=cd.id ";
            sqlcp += "           join " + schema + ".v_bhytll ll on ll.id=ds.id ";
            ...
            string sqlGroup = " select MAVAOVIEN,MABN,MAKP,TENKP,MADOITUONG,STTNHOM,NHOM,MAVP,TEN,DVT,DONGIA DONGIA,BHYT,KYTHUAT, TINHTONG,sum(SOLUONG) SOLUONG,sum(kq.sotien) TONGTIEN, sum(BHYTTRA) BHYTTRA,sum(SOTIEN-BHYTTRA) BNTRA ,tilethe,tilehuong,MIEN";
            sqlGroup += "           from ";
            sqlGroup += "           ( ";
            sqlGroup += sqlcp;
            sqlGroup += "           ) kq ";
            
            sqlGroup += "           group by MAVAOVIEN,MABN,MAKP,TENKP,MADOITUONG,STTNHOM,NHOM,MAVP,TEN,DVT,DONGIA,BHYT,KYTHUAT,TINHTONG,TILETHE,TILEHUONG ,MIEN";
            ...
            sql += " ba.chandoan  CHANDOAN,ba.maicd||' - (ICD 9) : '||ba.maicd9 MAICD,cd.sttnhom STTNHOM ,cd.nhom NHOM,  case when cd.bhyt=100 then 1 else 2 end STTLOAI,  ";
            ...
            sql += " case when cd.madoituong='1' and not (substr(ba.sothe,0,2)='GD' and ba.huongktc=0  and cd.kythuat=0 ) then TILEHUONG else 0 end BHYT,0 BHYT0117, ";
            sql += " case when cd.madoituong='7' then cd.TONGTIEN else 0 end TCSOTIEN,0 DICHSO,  ";
            ...
            sql += " 0	thanhtoandot,''	tenhc   ,to_char(TILETHE) TYLETHE,to_char(TILEHUONG)    TYLEHUONG   ,CD.MIEN MIEN ";
        ...
        public DataSet f_loadBCMau20(string tungay, string denngay, string dieukien, bool canngheo, string dsnhombhyt, string ThuocUT,string s_madoituong)
        {
            ...
            sql += " left join " + schema + ".BTDKP_BHYT BHKP ON BHKP.MAKP=X.MAKP ";
           // sql += " join BV115.v_bhytds ds on ds.id=ll.id ";
            ...
            sql += " where x.madoituong=1 and to_char(b.ngay,'yyyymmdd') between '{tungay}' and '{denngay}' and  x.idnhombhyt  in (@nhombhyt) and x.madoituong="+s_madoituong+" @dieukien";
        ...
        {
            string sql = "";
            sql = "	select nbhyt.ten tennhom,byt.id_byt DT_STT,byt.bv_apthau DT_BV,byt.ngayhieuluc DT_NGAY,byt.nhom_byt DT_NHOM,kq.mavp GHICHU ,byt.qd_stt masobyt,dm.tenhc TENHC,dm.ten TENVP,dm.duongdung DUONGDUNG,dm.dang DANGBAOCHE,dm.hamluong HAMLUONG,dm.dang DANGTRINHBAY,h.ten TENHANG,nc.ten TENNUOC,dm.sodk SOGIAYPHEP, dm.dang DONVI,kq.dongia GIAMUA,kq.dongia giattbh,  ";
            sql += "	SUM(kq.slngoaitru) SLNGOAITRU,sum(kq.slnoitru) SLNOITRU,	  ";
            sql += "	 kq.dongia*SUM(kq.slngoaitru) THANHTIEN_NGOAITRU,	  ";
            sql += "	 kq.dongia*SUM(kq.slnoitru) THANHTIEN_NOITRU	  ";
            sql += "	from (	  ";
            sql += "	 select ct.makp, ct.idnhombhyt idnhombhyt,ct.mavp mavp,round(ct.dongia,4) dongia,	  ";
            sql += "	 case when b.loaibn=1 then round(ct.soluong,2) else 0 end slngoaitru,	  ";
            sql += "	 case when b.loaibn=0 then round(ct.soluong,2) else 0 end slnoitru	  ";
            sql += "	 from " + schema + ".v_bhytct ct  	  ";
            sql += "	 join " + schema + ".v_bhytll b on b.id=ct.id 	  ";
          ...
          // sql += "	 join " + schema + ".BTDKP_BHYT BHKP ON BHKP.MAKP=kq.MAKP  	  ";
            ...
            sql += "	 join d_dmnhom nhomd on dm.manhom=nhomd.id  	  ";
            sql += "	 join v_nhomvp nhomvp on nhomd.nhomvp=nhomvp.ma 	  ";
            sql += "     left join bv115.d_dmbdbh byt on kq.mavp=byt.id ";
            sql += "	 inner join " + schema + ".v_nhombhyt nbhyt on nbhyt.id=kq.idnhombhyt 	  ";
            sql += "	 group by nbhyt.ten ,byt.id_byt ,byt.bv_apthau,byt.ngayhieuluc ,byt.nhom_byt ,kq.mavp ,byt.qd_stt  ,dm.ten , dm.hamluong,dm.tenhc,dm.dang,dm.duongdung,dm.sodk,nc.ten,h.ten,kq.dongia	   ";
        ...
        public DataSet f_loadBCMau20_XuatExcel_VTYTTL(string tungay, string denngay, string dieukien, bool canngheo, string dsnhombhyt, string ThuocUT, string s_madoituong)
        {
            string sql = "";
            sql = "	select nbhyt.ten tennhom,byt.mahoa DT_STT,byt.tendonvi DT_BV,byt.ngayhieuluc DT_NGAY,byt.phannhom DT_NHOM,kq.mavp GHICHU ,byt.stt_byt masobyt,dm.tenhc TENHC,dm.ten TENVP,dm.duongdung DUONGDUNG,dm.dang DANGBAOCHE,dm.hamluong HAMLUONG,dm.dang DANGTRINHBAY,h.ten TENHANG,nc.ten TENNUOC,dm.sodk SOGIAYPHEP, dm.dang DONVI,kq.dongia GIAMUA,kq.dongia giattbh,  ";
            sql += "	SUM(kq.slngoaitru) SLNGOAITRU,sum(kq.slnoitru) SLNOITRU,	  ";
            sql += "	 kq.dongia*SUM(kq.slngoaitru) THANHTIEN_NGOAITRU,	  ";
            sql += "	 kq.dongia*SUM(kq.slnoitru) THANHTIEN_NOITRU	  ";
            sql += "	from (	  ";
            ...
            sql += "	 case when b.loaibn=1 then round(ct.soluong,2) else 0 end slngoaitru,	  ";
            sql += "	 case when b.loaibn=0 then round(ct.soluong,2) else 0 end slnoitru	  ";
            sql += "	 from " + schema + ".v_bhytktc ct  	  ";
            sql += "	 join " + schema + ".v_bhytll b on b.id=ct.id 	  ";
            sql += "	 join " + schema + ".v_bhytds a on a.id=b.id  	  ";
            sql += "	 where     to_char(b.ngay,'yyyymmdd') between '{tungay}' and '{denngay}' 	  ";
            sql += "	 and ct.idnhombhyt in (@nhombhyt)   and ct.dongia<>0	 @dieukien ";
            sql += "	 ) kq	  ";
            // sql += "	 join " + schema + ".BTDKP_BHYT BHKP ON BHKP.MAKP=kq.MAKP  	  ";
            sql += "	 join d_dmbd dm on kq.mavp=dm.id  	";
            sql += "	 join d_dmnuoc nc on nc.id=dm.manuoc 	  ";
            ...
            sql += "	 join d_dmnhom nhomd on dm.manhom=nhomd.id  	  ";
            sql += "	 join v_nhomvp nhomvp on nhomd.nhomvp=nhomvp.ma 	  ";
            sql += "     left join bv115.d_duocbyt byt on kq.mavp=byt.mabd";
            sql += "	 inner join " + schema + ".v_nhombhyt nbhyt on nbhyt.id=kq.idnhombhyt 	  ";
            sql += "	 group by nbhyt.ten ,byt.mahoa ,byt.tendonvi,byt.ngayhieuluc ,byt.phannhom ,kq.mavp ,byt.stt_byt  ,dm.ten , dm.hamluong,dm.tenhc,dm.dang,dm.duongdung,dm.sodk,nc.ten,h.ten,kq.dongia	   ";
        ...
        {
            string sql = "";
            sql = "	select 0 stt,nbhyt.ten tennhom,byt.id_byt mahoa1,byt.qd_stt mahoa2,'ICD9 '||vp.maicd9||'#'||vp.id mavp,vp.ten tenvp, kq.dongia,	  ";
            sql+="	SUM(kq.slngoaitru) SLNGOAITRU,sum(kq.slnoitru) SLNOITRU,	  ";
            sql+="	 kq.dongia*SUM(kq.slngoaitru) THANHTIEN_NGOAITRU,	  ";
            sql+="	 kq.dongia*SUM(kq.slnoitru) THANHTIEN_NOITRU	  ";
            sql+="	from (	  ";
            ...
            sql += "	 inner join " + schema + ".v_nhombhyt nbhyt on nbhyt.id=kq.idnhombhyt 	  ";
            sql += "      left join " + schema + ".v_giavpbh byt on kq.mavp=byt.id ";
            sql += "	 group by nbhyt.ten ,byt.id_byt ,byt.qd_stt,'ICD9 '||vp.maicd9||'#'||vp.id ,vp.ten , kq.dongia	  ";
        ...
        public DataSet f_Export_Excel2(string tungay, string denngay, string dieukien)
        {
            ...
            sql += "  ,case when substr(a.sothe,-5,5)='79024' then 1 else  case when substr(substr(a.sothe,-5,5),0,2)='79' and substr(a.sothe,-5,5)!='79024' then 2 else 3 end end  MANHOMTHE, ";
            sql += "  case when substr(a.sothe,-5,5)='79024' then 'Bệnh nhân nội tỉnh, khám, chữa bệnh ban đầu' else  case when substr(substr(a.sothe,-5,5),0,2)='79' and substr(a.sothe,-5,5)!='79024' then 'Bệnh nhân nội tỉnh đến' else 'Bệnh nhân ngoại tỉnh đến' end end  NHOMTHE ";
            sql += " from " + schema + ".V_BHYTDS a ";
            sql += " join " + schema + ".V_BHYTLL b on  a.id=b.id ";
            sql += " join btdbn bn on bn.mabn= a.mabn ";
            sql += " join btdpxa px on bn.maphuongxa=px.maphuongxa ";
        ...
        {
            string sql = " select case when SUBSTR(a.SOTHE,-5,5)='79024' then '1.Bệnh nhân nội tỉnh, khám, chữa bệnh ban đầu' else  case when substr(SUBSTR(a.SOTHE,-5,5),0,2)='79' and SUBSTR(a.SOTHE,-5,5)<>'79024' then '2.Bệnh nhân nội tỉnh đến' else '3.Bệnh nhân ngoại tỉnh đến' end end  NHOMBAOCAO ,'BENH NHAN KCB' TUYENBAOCAO";
            ...
            sql += " join btdtt tt on bn.matt=tt.matt";
            sql += " inner join btdkp_bv kp on a.makp=kp.makp ";
            sql += " left join benhandt badt on badt.mavaovien=a.mavaovien ";
            sql += " where to_char(b.ngay,'yyyymmdd') between '{tungay}' and '{denngay}' @dieukien ";
            sql = sql.Replace("'{tungay}'", f_formatNgay(tungay));
            sql = sql.Replace("'{denngay}'", f_formatNgay(denngay));
            sql = sql.Replace("@dieukien", dieukien);
        ...
        public DataSet f_Export_Excel_M79_80_SYT(string tungay, string denngay, string dieukien)
        {
            ...
            sql += " ,to_char(A.TUNGAY,'yyyymmdd') gt_the_tu,to_char(A.DENNGAY,'yyyymmdd') gt_the_den,a.chandoan ten_benh ";
            ...
            sql += " ,CASE a.traituyen WHEN 0 THEN '1' ELSE '3' END ma_lydo_vvien,'1' ma_noi_chuyen,'1' ma_tai_nan ";
            sql += " ,to_char(A.NGAYVAO,'yyyymmddHH24mi ') NGAY_VAO,to_char(A.NGAYRA,'yyyymmddHH24mi') NGAY_RA ";
            sql += " ,case when ROUND(TO_DATE(TO_CHAR(A.NGAYRA,'DD/MM/YYYY'),'DD/MM/YYYY')-TO_DATE(TO_CHAR(A.NGAYVAO,'DD/MM/YYYY'),'DD/MM/YYYY'),1)=0  then 1 else ROUND(TO_DATE(TO_CHAR(A.NGAYRA,'DD/MM/YYYY'),'DD/MM/YYYY')-TO_DATE(TO_CHAR(A.NGAYVAO,'DD/MM/YYYY'),'DD/MM/YYYY'),1) end  so_ngay_dtri ";
         //   sql += " ,ROUND(TO_DATE(TO_CHAR(A.NGAYRA,'DD/MM/YYYY'),'DD/MM/YYYY')-TO_DATE(TO_CHAR(A.NGAYVAO,'DD/MM/YYYY'),'DD/MM/YYYY')) so_ngay_dtri ";
            //sql += " , B.XETNGHIEM T_XN ,b.CDHA+B.TDCN T_CDHA,b.THUOC T_THUOCDT,b.thuock T_THUOC_TILE,b.MAU T_MAU ";
            //sql += " ,b.DVKTTHUONG T_PTTT,b.VTTH  T_VTYT,b.VTYTTL T_VTYT_TILE ";
            //sql += "  ,b.DVKTCAO T_DVKTC_TILE,b.KHAMBENH+b.GIUONG T_KHAM,b.CPVC T_CPVC ";
            sql += " , xv.ketqua ket_qua_dtri,ttx.id_byt tinh_trang_rv,to_char(A.NGAYRA,'yyyymmddHH24mi') ngay_ttoan , b.tilehuong muc_huong";
            sql += "  ,b.THUOC t_thuoc,b.VTTH t_vtyt,b.sotien t_tongchi,round( b.sotien-b.bhyt,2) T_bntt, b.BHYT T_BHTT,B.UBNDTRA T_NGUONKHAC ";
            sql += " ,b.ngoaidinhsuat T_NGOAIDS ";
          //  sql += " ,'BVND115' NOIKCB,kp.tenkp as khoa";
            sql += " ,TO_CHAR(b.ngay,'YYYY') NAM_QT,TO_CHAR(b.ngay,'MM') THANG_QT,kpsyt.loai ma_loai_kcb,kpsyt.makpbo ma_khoa, '79024' ma_cskcb,'' ma_khuvuc,'' ma_pttt,'' can_nang";
           ...
           // sql += "  ,b.CHIPHI20_NGOAI T_TNT_NGOAI,b.CHIPHI31_NGOAI T_THAMPHAN_NGOAI,b.CHIPHI32_NGOAI T_THUOC_UT_NGOAI,b.KTUNGBUOU_NGOAI T_KYTHUATUB_NGOAI";
            ...
            sql += " join btdquan qu on bn.maqu=qu.maqu ";
            sql += " join btdtt tt on bn.matt=tt.matt";
            sql += " left join bv115.btdkp_bhyt kpsyt on kpsyt.makp=a.makp ";
            sql += " left join medibv115.xuatvien xv on xv.maql=a.maql ";
            sql += " left join bv115.ttxk_byt ttx on ttx.ma=xv.ttlucrv ";
            sql += " inner join btdkp_bv kp on a.makp=kp.makp ";
            sql += " where to_char(b.ngay,'yyyymmdd') between '{tungay}' and '{denngay}' @dieukien ";
            sql = sql.Replace("'{tungay}'", f_formatNgay(tungay));
            sql = sql.Replace("'{denngay}'", f_formatNgay(denngay));
        ...
        {
            string sql = " select a.mabn||'_'||b.sophieu sophieu,LOWER(bn.HOTEN) HOTEN,";
            ...
            sql += " CASE(INSTR( a.MAICD,';')) WHEN -1 THEN '' WHEN 0 THEN '' ELSE   SUBSTR(a.MAICD,INSTR( a.MAICD,';')+1,LENGTH(A.MAicd)- INSTR( a.MAICD,';')) END  BENHKHAC ";
            sql += " ,'_'||cn.MACN NOIKCB,kp.tenkp as khoa";
            sql += " ,TO_CHAR(b.ngay,'YY') NAM_QT,TO_CHAR(b.ngay,'MM') THANG_QT ";
            sql += " ,to_char(A.TUNGAY,'dd/mm/yyyy') GT_TU,to_char(A.DENNGAY,'dd/mm/yyyy') GT_DEN";
            sql += "  , bn.sonha||', '||bn.thon||', '||px.tenpxa||', '||qu.tenquan||', '||tt.tentt||'-MA_GD: '||cn.MACN DIACHI,";
            // sql += "  , bn.sonha||', '||bn.thon||', '||px.tenpxa||', '||qu.tenquan||', '||tt.tentt DIACHI,";
            sql += " '' TT_TNGT,a.mabn||'_'||b.sophieu MABN,a.chandoan CHANDOAN";
            sql += "  ,case when SUBSTR(a.SOTHE,-5,5)='79024' then 1 else  case when substr(SUBSTR(a.SOTHE,-5,5),0,2)='79' and SUBSTR(a.SOTHE,-5,5)<>'79024' then 2 else 3 end end  MANHOMTHE, ";
            sql += "  case when SUBSTR(a.SOTHE,-5,5)='79024' then 'Bệnh nhân nội tỉnh, khám, chữa bệnh ban đầu' else  case when substr(SUBSTR(a.SOTHE,-5,5),0,2)='79' and SUBSTR(a.SOTHE,-5,5)<>'79024' then 'Bệnh nhân nội tỉnh đến' else 'Bệnh nhân ngoại tỉnh đến' end end  NHOMTHE ";
            sql += " from " + schema + ".V_BHYTDS a ";
            sql += " join " + schema + ".V_BHYTLL b on  a.id=b.id ";
            ...
            sql += " join btdquan qu on bn.maqu=qu.maqu ";
            sql += " join btdtt tt on bn.matt=tt.matt";
            sql += " left join bv115.bhyt_cn cn on cn.maql=a.maql ";
            sql += " inner join btdkp_bv kp on a.makp=kp.makp ";
