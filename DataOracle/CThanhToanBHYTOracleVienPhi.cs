...
namespace DataOracle
{
    public class CThanhToanBHYTOracleVienPhi
    {
        ...
        {
            /*  Get so lieu  from DATA MEDISOFT - BANG VIEN PHI 
        ...
        public DataTable f_getv_ttrvkp_ct_ALL_FROMVPKTC(string MaBN, string idttrv, DateTime TuNgay, string loai)
        {
            ...
            sqlcp += "           join medibv115.btdkp_bv kp on ct.makp=kp.makp ";
            sqlcp += "           join bv115.btdkp_bhyt kpbh on kp.makp=kpbh.makp ";
            sqlcp += "             where (ct.madoituong=1 or (ct.id_ktcao>0 and ct.madoituong=2  and ct.gia_bh<>ct.dongia) or (ct.madoituong in (5,2) and ct.id_ktcao>0)) and kpbh.loaibcbh in ({loai},2)  ";
            sqlcp += "           and ds.id={idttrv}";
            sqlcp += " ) kq";
            sqlcp = sqlcp.Replace("{loai}", loai);
            sqlcp = sqlcp.Replace("{idttrv}", idttrv);
        ...
        {
            /*  Get so lieu vtyt from bv115.v_bhytktc 
        ...
        public DataTable f_getv_ttrvkp_ct_ALL_FROMVPKTC(string s_id)
        {
            /*  Get so lieu vtyt from bv115.v_bhytktc 
            ...
            string sqlcp;
            sqlcp = " select MAVP,TEN,DVT, SOLUONG,DONGIA DONGIA,kq.soluong*kq.dongia TONGTIEN,kq.BHYTTRA BHYTTRA,0 BNTRA,MADOITUONG,NGAY,ID,MAVAOVIEN,MAQL,MABN,MAKP,TENKP,IDKHOA,STTNHOM,NHOM,BHYT,KYTHUAT, TINHTONG,IDNHOMBHYT  ";
            sqlcp += " from (";
            sqlcp += "           select to_char(ct.ngay,'dd/mm/yyyy') ngay,ct.idtonghop ID,ds.mavaovien,ds.maql,ds.mabn,to_char(ct.makp) MAKP,kp.tenkp ,TO_CHAR(ct.id) idkhoa, ct.madoituong,  case when vp.kythuat=0 then -1 else nhom.stt end  sttnhom,  ";
            sqlcp += "         case when vp.kythuat=0 then 'Dịch vu kỹ thuật cao chi phí lớn' else to_char(nhom.ten) end nhom,ct.mavp, to_char(vp.ten) ten,to_char(vp.dvt) DVT,ct.soluong,ct.dongia,ct.bhyttra,vp.bhyt,vp.kythuat,2 tinhtong ,to_char(nhom.idnhombhyt) idnhombhyt";
            sqlcp += "           from " + schema + ".v_bhytds ds  ";
            sqlcp += "           join " + schema + ".v_bhytll ll on ds.id=ll.id     ";
            sqlcp += "           join " + schema + ".v_bhytktc ct on ct.id=ds.id     ";
            sqlcp += "           join medibv115.v_giavp vp on ct.mavp=vp.id ";
            sqlcp += "           join medibv115.v_loaivp loai on vp.id_loai=loai.id ";
            sqlcp += "           join medibv115.v_nhomvp nhom on loai.id_nhom=nhom.ma ";
            sqlcp += "           left join medibv115.btdkp_bv kp on ct.makp=kp.makp ";
            sqlcp += "             where ds.id='{s_id}' ";
            
            sqlcp += "           union all ";

            sqlcp += "           select to_char(ct.ngay,'dd/mm/yyyy') ngay,ct.idtonghop ID,ds.mavaovien,ds.maql,ds.mabn,to_char(ct.makp) MAKP,kp.tenkp ,TO_CHAR(ct.id) idkhoa, ct.madoituong,  case when vp.kythuat=0 then -1 else nhom.stt end  sttnhom,  ";
            sqlcp += "         case when vp.kythuat=0 then 'Dịch vu kỹ thuật cao chi phí lớn' else to_char(nhom.ten) end nhom,ct.mavp, to_char(vp.ten) ten,to_char(vp.dang) DVT,ct.soluong,ct.dongia,ct.bhyttra,vp.bhyt,vp.kythuat,2 tinhtong ,to_char(nhom.idnhombhyt) idnhombhyt";
            sqlcp += "           from " + schema + ".v_bhytds ds  ";
            sqlcp += "           join " + schema + ".v_bhytll ll on ds.id=ll.id     ";
            sqlcp += "           join " + schema + ".v_bhytktc ct on ct.id=ds.id     ";
            sqlcp += "           join medibv115.d_dmbd vp on ct.mavp=vp.id ";
        ...
        public DataTable f_getv_ttrvkp_ct_ALL_FROM_BHYTCT(string s_id)
        {
            /*  Get so lieu vtyt from bv115.v_bhytct             
            ...
            sqlcp += "           join medibv115.v_nhomvp nhom on loai.id_nhom=nhom.ma ";
            sqlcp += "           left join medibv115.btdkp_bv kp on ct.makp=kp.makp ";
            sqlcp += "             where ds.id='{s_id}' and ct.madoituong='5' ";
