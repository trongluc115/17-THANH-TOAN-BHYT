...
namespace DataOracle
{
    public class CDanhMucOracle
    {
        ...
        public  DataTable f_getDMDuoc_VienPhi()
        {
            ...
            string sql = "  select a.id id,rpad(lower(a.ten)||' '||a.hamluong||' ('||lower(a.DANG)||')   ',80)||lpad(TO_CHAR(ROUND(a.DONGIA,0)),15)  TEN,a.DANG||' ('||a.bhyt||') ' DONVI,ROUND(a.DONGIA,0) GIA_BH   from d_dmbd a  inner join d_dmnhom b on  a.manhom=b.id  where b.nhomvp <> 12";
            sql+=" UNION ALL ";
            sql += " select id id,rpad(lower(TEN)||' ('||lower(DVT)||')   ',80)||lpad(TO_CHAR(ROUND(GIA_BH_T3,0)),15) TEN,DVT||' ('||bhyt||')' DONVI,ROUND(GIA_BH,0) GIA_BH from V_GIAVP where id_loai  in (select id from v_loaivp where id_nhom not in (11) )  and gia_cu=0 ";
            try
            {
                ds = data.get_data(sql);
                dt=ds.Tables[0];
        ...
        public DataTable d_getnhomnhanvien()
        {
            AccessData data = new AccessData();
            DataSet ds = new DataSet();
            DataTable dt = new DataTable();

            string sql = "select id id,ten ten from nhomnhanvien order by ten ";
        ...
        public DataTable d_gettrinhdo()
        {
            AccessData data = new AccessData();
            DataSet ds = new DataSet();
            DataTable dt = new DataTable();

            string sql = "select id id,ten ten from bv115.trinhdo order by ten ";
        ...
        public DataTable d_getchuyenkhoa()
        {
            AccessData data = new AccessData();
            DataSet ds = new DataSet();
            DataTable dt = new DataTable();

            string sql = "select id id,ten ten from dmchuyenkhoa order by ten ";
        ...
        public DataTable d_getDMDT_LOAI(string s_idnhom)
        {
            AccessData data = new AccessData();
            DataSet ds = new DataSet();
            DataTable dt = new DataTable();

            string sql = "select ID id,ten ten from bv115.dt_loai where hide<>'1' ";
            if (s_idnhom.Length > 0)
            {
                sql += " and id_nhom in (" + s_idnhom + ") ";
            }
            sql += " order by ten ";
            try
            {
        ...
        public DataTable d_getDMKhoaPhong(string s_value)
        {
         ...
         public DataTable d_getDMKhoaPhongFromLoai(string s_value)
        {
            AccessData data = new AccessData();
            DataSet ds = new DataSet();
            DataTable dt = new DataTable();
            ...
            if (s_value.Length > 0)
            {
                sql = sql.Replace("{dieukien}", " and loai in (" + s_value + ")");
            }
            else
            {
                sql = sql.Replace("{dieukien}", "");
        ...
        public double f_getgiaVP(string mavp)
        {
            ...
            sql+=" union select round(gia_bh_t3,2) dongia from v_giavp where id=@mavp ";
