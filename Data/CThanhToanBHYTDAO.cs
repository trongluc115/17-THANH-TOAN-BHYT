...
namespace Data
{
    public class CThanhToanBHYTDAO
    {
        ...
        public string get_rpt_ttravien_kp_ct(string MaBN,string mavaovien)
        {
            DataSet ds = new DataSet();
            string s = "";
            try
        ...
        public int Insert(CThanhToanBHYT t)
        {
            ...
            try
            {
                sql = "Insert into thanhtoanbhyt(mabn,nhomthe,loaithe,mavaovien,sothebhyt,noidangky,icd,chandoan,tongtien,bhyttra,bntra,thuoc,mau,xetnghiem,cdha,dvktthongthuong,dvktcao,vtyt,tienkham,chiphivc,thamdochucnang,giuong,traituyen,idttrv,ngaylamviec,sophieu,khac,manhomthe,ngayhd,loaiba,sobienlai,tungay)";
                ...
                sql += t.ThamDoChucNang + "," + t.Giuong + "," + t.TraiTuyen + "," + t.IDTTRV + ",'" + FormatYYYY_MM_DD(t.NgayLamViec) + "','" + t.SoPhieu + "'," + t.Khac + ",'" + t.MaNhomThe + "','" + FormatYYYY_MM_DD(t.HSD) + "','" + t.LoaiBA + "'," + t.SoBienLai + ",'" + FormatYYYY_MM_DD(t.TuNgay) + "')";
                CConnection data = new CConnection();
        ...
        public int InsertThuoc20(string id, string mabn, string stt, string mabd, string dongia, string soluong, string thanhtien, string GPNK, string BVAPTHAU, string noikham, DateTime ngay)
        {
            int roweffect = 0;
            try
            {
                sql = "Insert into d_thuocbhyt20(ID,STT,MABN,MABD,DONGIA,SOLUONG,THANHTIEN,GPNK,BVAPTHAU,MaKho,ngay)";
                sql += " values(" + id + "," + stt + ",'" + mabn + "','" + mabd + "',";
                sql += dongia + "," + soluong + "," + thanhtien + ",'" + GPNK + "','" + BVAPTHAU + "','" + noikham + "','" + FormatYYYY_MM_DD(ngay) + "')";
        ...
        public int InsertCLS20(string id, string mabn, string stt, string mavp, string dongia, string soluong, DateTime ngay)
        {
            int roweffect = 0;
            try
            {
                sql = "Insert into d_clsbhyt20(ID,STT,MABN,MAVP,DONGIA,soluong,ngay)";
                sql += " values(" + id + "," + stt + ",'" + mabn + "','" + mavp + "',";
                sql += dongia + "," + soluong + ",'" + FormatYYYY_MM_DD(ngay) + "')";
